...
export function MainContainer({
      ...
      timeInterval = setInterval(() => {
        if (roundTime.seconds !== 0) {
          ...
          if (roundTime.minutes !== 0) {
            setRoundTime({
              minutes: roundTime.minutes - 1,
              seconds: roundTime.seconds + 59,
            });
          } else {
  ...
  useEffect(() => {
    ...
    consonantsInAnswer.forEach(
      (c) => chosenLetters.includes(c) && remainingConsonants.push(c),
    );
    ...
    vowelsInAnswer.forEach(
      (c) => chosenLetters.includes(c) && remainingVowels.push(c),
    );
    ...
    if (remainingConsonants.length >= consonantsInAnswer.length) {
      setDisabledButtonsState({
        ...
        spinButton: true,
        buyVowelButton:
          gamePoints.roundPoints >= 400 &&
          !(remainingVowels.length >= vowelsInAnswer.length)
            ? false
            : true,
      });
      ...
      setDisabledButtonsState({
        ...disabledButtonsState,
        spinButton:
          remainingConsonants.length >= consonantsInAnswer.length
            ? true
            : false,
        buyVowelButton: true,
      });
  ...
  function resetRound() {
    ...
    if (
      ...
      setGamePoints({
        roundPoints: 0,
        totalPoints: gamePoints.totalPoints + gamePoints.roundPoints,
      });
    }
  ...
  function handleVowels(letter) {
    function chooseVowelsToReveal(letter) {
      setGamePoints({
        ...gamePoints,
        roundPoints: gamePoints.roundPoints - 400,
      });
      setChosenLetters((prev) => [...prev, letter]);
  ...
  return (
    <RoundStateContext.Provider value={round}>
      {!showEndGamePanel ? (
        <div className="3xl:my-32 my-8 grid grid-cols-1 grid-rows-2 lg:my-20 lg:grid-cols-2 lg:grid-rows-1 lg:gap-8 xl:gap-16 2xl:gap-24">
          ...
          <DisabledButtonsStateContext.Provider
            value={[disabledButtonsState, setDisabledButtonsState]}
          >
            <WheelSide
